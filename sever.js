...
app.post("/register", (req, res) => {
    ...
    Users.findOne({where:{ username: req.body.username }}).then(existingUser => {
        if (existingUser) {
            ...
            Users.create(req.body).then(newUser => {
                ...
                console.error(error);
                res.status(500).send('An error occurred creating a new user.');
            });
        }
    }).catch(error => {
        console.error(error);
...
app.post('/Webadmin', async (req, res) => {
    try {
        const { title, author, image, typeid } = req.body;
